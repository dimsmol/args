...
Option.prototype.parse = function (optionInfo) {
	...
	this.defaultValue = optionInfo.defaultValue;
	...
	if (this.defaultValue) {
		try {
			this.parsedDefaultValue = this.parseValue(this.defaultValue);
		}
		catch (err) {
			if (err instanceof errors.BadValue) {
				throw new Error([
					'Bad default value "', this.defaultValue,
					'" for option "', this.getName(), '"'].join(''));
			}
			else {
				throw err;
			}
